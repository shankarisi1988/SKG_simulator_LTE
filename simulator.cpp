...
void run_simulation()
 {
  ...
  //   cuser=0;
       			  ...
       			  for(int j=0; j<5; j++)
        			  {
							 ...
							 ho_thr=base[j].hothreshold();
										...
										/*Estimating data rate from target network*/
										     ...
										     else if (enable_DC)    /*Dual connectivity is enabled between base station j and target_id*/
											{
											     target_id=base[j].network_selection(i);
 									...
 									if((data_rate_current >= DATA_RATE))
											 ...
											 if((data_rate_current <= DATA_RATE) && ((data_rate_current+data_rate_target) >= DATA_RATE))
											        	{
														pdc=pdc+1;
												  		 if((subcarrier[j] < SUBCARRIER) && (subcarrier[target_id] < SUBCARRIER))
															{
                                                                                                                 std::random_device rd;
	     													 ...
	     													 std::poisson_distribution<int> dist(CBR);
   													         ...
   													         call_arrival_probability=dist(e2);
										ob[i].throughput=ob[i].throughput+(data_rate_current + data_rate_target)*call_arrival_probability;
												...
												if (data_rate_current + data_rate_target < DATA_RATE)
												{
								             ...
								             base[j].execute_softer(i,j, target_id, data_rate_target, data_rate_current);  // execute DC
											  }
   ...
   cout << "Call dropping probability: " << (temp_outage+call_drop)/(temp_timer*CBR) << "\n";
 }
